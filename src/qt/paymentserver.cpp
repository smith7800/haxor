...
const int HAXOR_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString HAXOR_IPC_PREFIX("haxor:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(HAXOR_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(HAXOR_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(HAXOR_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start haxor: click-to-pay handler");
    else
